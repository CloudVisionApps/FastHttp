# FastHTTP Advanced Documentation

Complete technical documentation for FastHTTP web server.
...
FastHTTP supports virtual hosts, allowing you to serve multiple websites from a single server. Here's a comprehensive example configuration:
...
**Convert Apache configuration:**
```bash
./fasthttp convert --from apache --input /path/to/httpd.conf --output fasthttp.json
```
